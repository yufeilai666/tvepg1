...
def parse_hollywood_schedule_html(html_content):
    ...
    while i < len(date_divs):
        ...
        if date_match:
            ...
            programs = []
            j = i + 1
            while j < len(date_divs):
                next_div = date_divs[j]
                next_h6 = next_div.find('h6', class_='font_6')
                if not next_h6:
                    j += 1
                    continue
                    
                next_text = next_h6.get_text(strip=True)
                
                # 如果遇到下一个日期，停止收集节目
                if re.search(r'\d{1,2}/\d{1,2}\s+星期', next_text):
                    break
                
                # 检查是否为节目列表
                if re.search(r'\d{1,2}:\d{2}', next_text):
                    # 解析节目列表
                    full_text = next_h6.get_text()
                    ...
                    for k, line in enumerate(lines):
                        ...
                        # 确定节目日期
                        if k > 0 and not has_crossed_midnight:
                            # 找到前一个有效节目的时间
                            for m in range(k-1, -1, -1):
                                prev_line = re.sub(r'[\u200b\u200c\u200d\ufeff]', '', lines[m].strip())
            ...
            if programs:
                ...
                print(f"日期 {date_str}: {len(programs)} 个节目")
